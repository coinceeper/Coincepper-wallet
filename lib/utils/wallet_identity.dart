...
Future<String?> deriveEthStyleAddressFromMnemonic(String mnemonic) async {
  try {
    ...
    SecureLog.w('WalletIdentity: mnemonic derivation failed', error: e);
    return null;
  }
}
...
Future<String?> _readCachedBitcoinAddress(String userId) async {
  ...
  try {
    ...
    SecureLog.w('WalletIdentity: failed to decode cached bitcoin address', error: e);
    return null;
  }
}

Future<String?> _fetchBitcoinAddressFromCoinCeeper(String userId) async {
  final fromRegistry = await ServiceLocator.get<AddressRegistry>().addressForChain(
    userId,
    _kBtcBlockchainName,
  );
  ...
  if (walletName != null) {
    await ServiceLocator.get<AddressRegistry>().refreshFromSecureStorage(
      walletName: walletName,
      userId: userId,
    );
    return ServiceLocator.get<AddressRegistry>().addressForChain(userId, _kBtcBlockchainName);
  }
  return null;
}
