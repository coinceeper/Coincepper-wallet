...
///
/// Uses [anonymousId] instead of [userId] so the proxy server never
/// learns the actual user identity. The anonymous ID is a locally-
/// generated UUID stored in SecureStorage.
...
/// Price alert item returned from GET /api/notifications/price-alerts/{DeviceID}.
...
class BulkPricesResponse {
  ...
  factory BulkPricesResponse.fromJson(Map<String, dynamic> json) {
    ...
    Map<String, dynamic> rawPrices;
    if (pricesRaw is Map) {
      rawPrices = pricesRaw.map((k, v) => MapEntry(k.toString(), v));
    } else {
      rawPrices = <String, dynamic>{};
    }
...
/// Response for GET /api/notifications/price-alerts/{DeviceID}.
...
class FcmDataPayload {
  ...
  factory FcmDataPayload.fromMap(Map<String, dynamic> data) {
    return FcmDataPayload(
      ...
      price: _parseNum(data['price'])?.toDouble(),
      targetPrice: _parseNum(data['target_price'])?.toDouble(),
  ...
  /// Parse a value that could be a [num] or [String] into [num].
  /// FCM data maps contain only strings, so we need to handle both.
