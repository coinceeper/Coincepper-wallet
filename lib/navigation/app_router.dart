...
class AppRouter {
  ...
  /// Holds pending version status (set in main() before runApp).
  /// The /force-update route builder reads this.
  static VersionStatus? pendingVersionStatus;
  ...
  static final GoRouter router = GoRouter(
    ...
    refreshListenable: ServiceLocator.get<AppNavigationState>(),
    observers: [ServiceLocator.get<SensitiveRouteObserver>()],
    redirect: _redirect,
    ...
    routes: [
      ...
      // Force update screen (full page, not dismissible if force)
      GoRoute(
        path: RoutePaths.forceUpdate,
        pageBuilder: (c, s) {
          final status = (s.extra as VersionStatus?) ?? AppRouter.pendingVersionStatus;
          return platformPage(
          ...
          return platformPage(
            child: SensitiveScreenScope(
              child: PasscodeScreen(
                ...
                onSuccess: () {
                  final callback =
                      ServiceLocator.get<WalletSession>().consumePasscodeOnSuccess();
                  if (callback != null) {
                    callback();
                    return;
                  }
      ...
      GoRoute(
        ...
        pageBuilder: (c, s) {
          final extra = s.extra as Map<String, dynamic>?;
          final queryParams = s.uri.queryParameters;
          return platformPage(
            child: WebViewScreen(
      ...
      GoRoute(
        ...
        pageBuilder: (c, s) {
          final extra = s.extra as Map<String, dynamic>?;
          final mnemonic = ServiceLocator.get<WalletSession>().consumeMnemonic() ??
      ...
      // Route for notification deep links: /crypto-details/ETH (path param variant)
      GoRoute(
        path: '${RoutePaths.cryptoDetails}/:symbol',
        pageBuilder: (c, s) {
          final symbol = s.pathParameters['symbol'] ?? '';
          final extra = CryptoDetailsExtra.from(s.extra);
          return platformPage(
            child: CryptoDetailsScreen(
              tokenName: extra?.tokenName ?? symbol,
              tokenSymbol: extra?.tokenSymbol ?? symbol,
              iconUrl: extra?.iconUrl ?? '',
              isToken: extra?.isToken ?? false,
              blockchainName: extra?.blockchainName ?? symbol,
              gasFee: extra?.gasFee ?? 0.0,
            ),
            state: s,
          );
      ...
      // Route for notification deep links: /transaction_detail/abc123 (path param variant)
      GoRoute(
        path: '${RoutePaths.transactionDetail}/:id',
        pageBuilder: (c, s) {
  ...
  static Future<String?> _redirect(BuildContext context, GoRouterState state) async {
    final nav = ServiceLocator.get<AppNavigationState>();
    ...
    if (nav.sessionLockRequired && matched != RoutePaths.enterPasscode) {
      SecureLog.d('🔄 ROUTER REDIRECT: Session lock required -> redirect to enter-passcode');
    ...
    if (shouldLock &&
      ...
      SecureLog.d('ROUTER REDIRECT: Background lock required -> redirect to enter-passcode');
      await SessionLockCoordinator.saveReturnUri(loc);
      return RoutePaths.enterPasscode;
    }
    return null;
