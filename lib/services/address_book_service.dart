...
class AddressBookService implements IAddressBookService {
  ...
  @override
  Future<void> saveToKeystore(String walletName, String walletAddress) async {
    await saveWalletToKeystore(walletName, walletAddress);
  }

  @override
  Future<List<Map<String, String>>> loadFromKeystore() async {
    final entries = await loadWalletsFromKeystore();
    return entries.map((e) => {'name': e.name, 'address': e.address}).toList();
  }

  @override
  Future<void> deleteFromKeystore(String walletName) async {
    await deleteWalletFromKeystore(walletName);
  }
