...
class CryptoLogoCacheService {
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      if (cacheData != null && now.difference(cacheTime) < _cacheExpiry) {
        ...
        SecureLog.w('Logo cache expired or empty, will refresh from API');
  ...
  static Future<String?> getLogoUrl(String symbol, {String? blockchain}) async {
    ...
    if (_logoCache.containsKey(cacheKey)) {
      SecureLog.i('Logo cache hit for $cacheKey');
      return _logoCache[cacheKey];
    }

    // Try symbol only
    if (_logoCache.containsKey(symbol)) {
      SecureLog.i('Logo cache hit for $symbol');
      return _logoCache[symbol];
    }

    SecureLog.e('Logo cache miss for $symbol (blockchain: $blockchain)');
  ...
  static Future<void> _refreshCacheFromAPI() async {
    try {
      SecureLog.d('Refreshing logo cache from API...');

      final apiService = ServiceLocator.get<ServiceProvider>().apiService;
      ...
      if (response.success && response.currencies.isNotEmpty) {
        ...
        SecureLog.i('Logo cache refreshed: ${_logoCache.length} logos cached');
        SecureLog.d('Cached symbols: ${_logoCache.keys.take(10).join(', ')}...');
      } else {
        SecureLog.w('Failed to refresh logo cache from API');
      }
    } catch (e) {
          ...
          child: SizedBox(
            ...
            child: isLoading
              ...
              errorWidget: (context, url, error) {
                SecureLog.e('Error loading cached logo', error: error);
                return Icon(
