...
/// @Deprecated('Replaced by CoinGeckoService — this file is kept for legacy compatibility only')
@Deprecated('Use CoinGeckoService instead')
class ChartApiService {
  // This file is kept for legacy compatibility. All methods return null.
  ...
  static Future<ChartApiData?> getChartData({
    ...
    try {
      SecureLog.i('Fetching chart data');
      ...
      SecureLog.i('Chart API Response Status: ${response.statusCode}');

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        SecureLog.i('Chart API Response Keys: ${data.keys}');

        if (data['success'] == true && data['chart_data'] != null) {
          final chartApiData = ChartApiData.fromJson(data);
          SecureLog.i('Chart data parsed successfully: ${chartApiData.data.length} data points');
          return chartApiData;
        } else {
          final errorMessage = data['message'] ?? 'Unknown error';
          SecureLog.e('Chart API Error');
          SecureLog.i('API Response received');
        }
      } else {
        SecureLog.e('Chart API HTTP Error: ${response.statusCode}');
        SecureLog.i('Response Body received');
      }
      return null;
    } catch (e) {
      SecureLog.e('Error fetching chart data', error: e);
      return null;
    }
  }
  ...
  static Future<Map<String, LivePriceData>?> getLivePrices({
    ...
    try {
      SecureLog.i('Fetching live prices');
      ...
      SecureLog.i('Live Price API Response Status: ${response.statusCode}');

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        SecureLog.i('Live Price API Response Keys: ${data.keys}');
        ...
        if (data['success'] == true && data['live_prices'] != null) {
          ...
          for (final entry in pricesData.entries) {
            try {
              ...
              SecureLog.i('Live price parsed');
            } catch (parseError) {
              SecureLog.e('Error parsing live price', error: parseError);
            }
          }

          SecureLog.i('Live prices loaded for ${livePrices.length} symbols');
          return livePrices;
        } else {
          final errorMessage = data['message'] ?? 'Unknown error';
          SecureLog.e('Live Price API Error');
          SecureLog.i('API Response received');
        }
      } else {
        SecureLog.e('Live Price API HTTP Error: ${response.statusCode}');
        SecureLog.i('Response Body received');
      }
      return null;
    } catch (e) {
      SecureLog.e('Error fetching live prices', error: e);
      return null;
    }
  }
  ...
  static int _getPointsForTimeframe(String timeframe) {
    switch (timeframe) {
      case '1h':
        return 60; // 60 points for hourly data (last 60 minutes, 1-minute intervals)
      case '1d':
        return 24; // 24 points for daily data (last 24 hours, 1-hour intervals)
      case '1w':
        return 168; // 168 points for weekly data (7 days × 24 hours, 1-hour intervals)
      case '1m':
        return 30; // 30 points for monthly data (last 30 days, 1-day intervals)
      case '3m':
        return 90; // 90 points for 3-month data (last 90 days, 1-day intervals)
      case '1y':
        return 52; // 52 points for yearly data (last 52 weeks, 1-week intervals)
      default:
        return 30;
    }
  }
