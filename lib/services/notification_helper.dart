...
/// - Local notification display (foreground + background)
/// - Notification tap handling + deep link navigation
/// - Permission requests
class NotificationHelper {
  static final FlutterLocalNotificationsPlugin _notifications = FlutterLocalNotificationsPlugin();

  /// The last notification's deep-link route, stored so the tap callback
  /// can navigate the user to the correct screen.
  static String? _lastNotificationRoute;

  /// External navigation callback — set by [FirebaseMessagingService]
  /// so that tapping a local notification navigates to the right screen.
  /// Cannot import [AppRouter] directly (would create coupling), so we use
  /// a lightweight function pointer instead.
  static void Function(String route)? navigateToRoute;
  ...
  /// Store the deep-link route for the last shown notification so the
  /// [onDidReceiveNotificationResponse] callback can navigate to it
  /// when the user taps the local notification.
  static void storeLastNotificationRoute(String? route) {
    _lastNotificationRoute = route;
  }
  ...
  static Future<void> initialize() async {
    try {
      ...
      // When user taps a local notification (in foreground or background),
      // navigate to the deep-link route that was stored when the notification
      // was shown via [storeLastNotificationRoute].
      void onNotificationResponse(NotificationResponse response) {
        SecureLog.d('Local notification tapped: ${response.payload}');
        final route = _lastNotificationRoute;
        if (route != null && route.isNotEmpty) {
          navigateToRoute?.call(route);
        }
      }
      ...
      SecureLog.e('NotificationHelper.initialize skipped', error: e, stackTrace: st);
    }
  }
    ...
    if (Platform.isAndroid) {
      ...
      if (android != null) {
        ...
        await android.createNotificationChannel(AndroidNotificationChannel(
          ...
          sound: const RawResourceAndroidNotificationSound('receive_sound'),
        ));
        await android.createNotificationChannel(AndroidNotificationChannel(
          sendChannelId,
          ...
          importance: Importance.high,
          sound: const RawResourceAndroidNotificationSound('send_sound'),
        ));
        await android.createNotificationChannel(AndroidNotificationChannel(
          welcomeChannelId,
          ...
          sound: const RawResourceAndroidNotificationSound('welcome_sound'),
        ));
        await android.createNotificationChannel(AndroidNotificationChannel(
          priceAlertChannelId,
          ...
          importance: Importance.high,
          sound: const RawResourceAndroidNotificationSound('price_alert_sound'),
        ));
        await android.createNotificationChannel(AndroidNotificationChannel(
          loginChannelId,
  ...
  static Future<void> initializeNotificationSettings() async {
    try {
      SecureLog.i('Initializing notification settings');
      if (Platform.isIOS || Platform.isAndroid) {
        final permission = await Permission.notification.request();
        if (permission.isGranted) {
          SecureLog.i('Notification permission granted');
        } else {
          SecureLog.i('Notification permission denied');
        }
      }
      SecureLog.i('Notification settings initialized');
    } catch (e) {
      SecureLog.e('Error initializing notification settings', error: e);
    }
  }
}
