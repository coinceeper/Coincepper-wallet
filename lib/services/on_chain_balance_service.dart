...
///
/// ## استراتژی Hybrid:
/// 1. **Proxy (Primary)**: POST /api/v2/balance/native, /api/v2/balance/token
/// 2. **Direct (Fallback)**: از RPC عمومی یا API مستقیم (همین الان)
class OnChainBalanceService {
  OnChainBalanceService._();
  /// DI constructor. Use [instance] for singleton access.
  OnChainBalanceService();
  static OnChainBalanceService get instance => ServiceLocator.get<OnChainBalanceService>();
  ...
  /// Proxy-aware EVM native balance.
  Future<double?> _evmBalance(String chain, String address) async {
    return ServiceLocator.get<BackendProxyService>().route(
      endpoint: 'balance/native/$chain',
      proxyCall: () => _evmBalanceFromProxy(chain, address),
      directCall: () => _evmBalanceDirect(chain, address),
    );
  }

  Future<double> _evmBalanceFromProxy(String chain, String address) async {
    final response = await ServiceLocator.get<BackendProxyService>()
        .proxyPost('balance/native', body: {
    ...
    if (json == null || json['status'] != 'ok') {
      throw Exception('Proxy native balance failed');
    }
    final balance = json['balance'] as num? ?? 0;
    return balance.toDouble();
  }

  Future<double> _evmBalanceDirect(String chain, String address) async {
  ...
  /// Proxy-aware BTC balance.
  Future<double?> _btcBalance(String address) async {
    try {
      return await ServiceLocator.get<BackendProxyService>().route(
        endpoint: 'balance/native/btc',
        proxyCall: () => _btcBalanceFromProxy(address),
        directCall: () => _btcBalanceDirect(address),
      );
    } catch (e) {
      SecureLog.w('OnChainBalance: BTC balance fetch failed', error: e);
      return null;
    }
  }

  Future<double> _btcBalanceFromProxy(String address) async {
    final response = await ServiceLocator.get<BackendProxyService>()
        .proxyPost('balance/native', body: {
      'chain': 'bitcoin',
      'address': address,
    ...
    final json = BackendProxyService.parseJson(response);
    if (json == null || json['status'] != 'ok') {
      throw Exception('Proxy BTC balance failed');
    }
    return (json['balance'] as num?)?.toDouble() ?? 0.0;
  }

  Future<double> _btcBalanceDirect(String address) async {
    for (final key in BuildSecrets.blockcypherApiKeys) {
      try {
        final res = await http
            .get(
              Uri.parse(
                  'https://api.blockcypher.com/v1/btc/main/addrs/$address/balance?token=$key'),
            )
        ...
        SecureLog.w('OnChainBalance: BlockCypher BTC balance attempt failed, trying next', error: e);
        continue;
      }
    }
    ...
    try {
      ...
      if (res.statusCode != 200) throw Exception('Blockstream failed');
      final map = jsonDecode(res.body) as Map<String, dynamic>;
      final funded = (map['chain_stats']?['funded_txo_sum'] as num?) ?? 0;
      final spent = (map['chain_stats']?['spent_txo_sum'] as num?) ?? 0;
      return (funded - spent) / 1e8;
    } catch (e) {
      SecureLog.w('OnChainBalance: Blockstream BTC balance failed after BlockCypher retries', error: e);
      throw Exception('All BTC balance sources failed');
    }
  }

  // ── ERC20 / TRC20 Token balances ──────────────────────────
  ...
  Future<double?> _erc20Balance(
    ...
    try {
      return await ServiceLocator.get<BackendProxyService>().route(
        endpoint: 'balance/token/$chain',
        proxyCall: () => _erc20BalanceFromProxy(chain, holder, contract),
        directCall: () => _erc20BalanceDirect(chain, holder, contract, decimals),
      );
    } catch (e) {
      SecureLog.w('OnChainBalance: ERC20 balance fetch failed for $chain', error: e);
      return null;
    }
  }

  Future<double> _erc20BalanceFromProxy(
      String chain, String holder, String contract) async {
    final response = await ServiceLocator.get<BackendProxyService>()
        .proxyPost('balance/token', body: {
      'chain': chain.toLowerCase(),
      'address': holder,
      'contract': contract,
    });
    ...
    final json = BackendProxyService.parseJson(response);
    if (json == null || json['status'] != 'ok') {
      throw Exception('Proxy token balance failed');
    }
    return (json['balance'] as num?)?.toDouble() ?? 0.0;
  }

  Future<double> _erc20BalanceDirect(
    String chain,
    String holder,
    String contract,
    int decimals,
  ...
  // ── Non-EVM balances (direct only, no proxy benefit yet) ──
  ...
  Future<double?> _trc20Balance(
    ...
    try {
      ...
      SecureLog.w('OnChainBalance: batch ERC20 balance lookup failed', error: e);
      return null;
    }
  }
  ...
  Future<double?> _tronBalance(String address) async {
    try {
      ...
      SecureLog.w('OnChainBalance: TRON balance fetch failed', error: e);
      return null;
    }
  }
  ...
  Future<double?> _xrpBalance(String address) async {
    ...
    final map = jsonDecode(res.body) as Map<String, dynamic>;
    final drops =
        map['result']?['account_data']?['Balance']?.toString() ?? '0';
