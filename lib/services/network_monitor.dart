...
class NetworkMonitor extends ChangeNotifier {
  static NetworkMonitor get instance => ServiceLocator.get<NetworkMonitor>();

  /// DI constructor. Use [instance] for singleton access.
  NetworkMonitor() : this._internal();
  ...
  void _initConnectivity() async {
    try {
      ...
      SecureLog.e('Error checking connectivity', error: e);
    }
  }
  ...
  Future<void> showNetworkStatus() async {
    SecureLog.i('Network Status - Connected: $_isConnected, Internet: $_isInternetAvailable, Type: $_connectionType, Quality: $_connectionQuality');
  }
  ...
  Stream<bool> get isOnlineStream {
    return Stream.periodic(const Duration(seconds: 5), (_) => isOnline);
  }
