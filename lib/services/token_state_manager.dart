...
/// DEPRECATED — TokenPreferences + BalanceManager are now the single source of truth.
///
/// Previously this class attempted to be a "single source of truth" for token state,
/// but it created a THIRD independent storage location (SharedPreferences keys
/// `token_state_*`, `balance_cache_*`) alongside TokenPreferences and SecureStorage
/// without any atomicity guarantee.
///
/// Token enabled/disabled state is now managed exclusively by [TokenPreferences]
/// (wallet-scoped, atomic writes). Balance data is managed by BalanceManager.
/// This class is kept for reference but is no longer registered in DI or used.
class TokenStateManager extends ChangeNotifier {
  TokenStateManager();

  TokenStateManager._();

  static TokenStateManager get instance => ServiceLocator.get<TokenStateManager>();
  ...
  Future<void> initialize(String userId, String walletName) async {
    SecureLog.i('TokenStateManager: Initializing');
    ...
    SecureLog.i('TokenStateManager: Initialized successfully');
  }
  ...
  Future<bool> toggleToken(String userId, CryptoToken token, bool isActive) async {
    SecureLog.i('TokenStateManager: Toggling ${token.symbol} to $isActive');
    ...
    try {
      ...
      SecureLog.i('TokenStateManager: Token ${token.symbol} toggled to $isActive');
      return true;
      
    } catch (e) {
      SecureLog.e('TokenStateManager: Error toggling token', error: e);
      return false;
    } finally {
  ...
  Future<void> updateUserTokens(String userId, List<CryptoToken> tokens) async {
    SecureLog.i('TokenStateManager: Updating ${tokens.length} tokens');
    ...
    try {
      ...
      SecureLog.i('TokenStateManager: Updated ${tokens.length} tokens, ${activeSet.length} active');
  ...
  Future<void> updateBalances(String userId, Map<String, double> balances) async {
    SecureLog.i('TokenStateManager: Updating ${balances.length} balances');
    ...
    try {
      ...
      SecureLog.i('TokenStateManager: Updated ${balances.length} balances');
  ...
  Future<void> clearUserData(String userId) async {
    SecureLog.i('TokenStateManager: Clearing data for current user');
    ...
    try {
      ...
      SecureLog.i('TokenStateManager: Cleared data');
  ...
  Future<void> _loadUserTokenState(String userId, String walletName) async {
    try {
      ...
      SecureLog.i('TokenStateManager: Loaded state');
      
    } catch (e) {
      SecureLog.e('TokenStateManager: Error loading user state', error: e);
    }
  }
  ...
  Future<void> _persistUserTokenState(String userId) async {
    try {
      ...
      SecureLog.i('TokenStateManager: Persisted token state');
      
    } catch (e) {
      SecureLog.e('TokenStateManager: Error persisting token state', error: e);
    }
  }
  ...
  Future<void> _persistBalanceCache(String userId) async {
    try {
      ...
      SecureLog.i('TokenStateManager: Persisted balance cache');
      
    } catch (e) {
      SecureLog.e('TokenStateManager: Error persisting balance cache', error: e);
    }
  }
  
  /// Debug method
  void debugState() {
    SecureLog.i('=== TokenStateManager Debug ===');
    SecureLog.i('Current Wallet: $_currentWalletName');
    SecureLog.i('Users with tokens: ${_userTokens.length}');
    SecureLog.i('Users with active tokens: ${_userActiveTokens.length}');
    SecureLog.i('Users with balances: ${_userBalances.length}');
    ...
    if (_currentUserId != null) {
      ...
      SecureLog.i('Current user tokens: ${tokens.length}');
      SecureLog.i('Current user active: ${active.length}');
      SecureLog.i('Current user balances: ${balances.length}');
    }
    SecureLog.i('==============================');
  }
}
