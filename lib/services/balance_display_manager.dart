...
class BalanceDisplayManager extends ChangeNotifier {
  ...
  static BalanceDisplayManager get instance => ServiceLocator.get<BalanceDisplayManager>();
  ...
  String _formatBalanceForScreen(double balance, bool isSmallScreen, bool isMediumScreen) {
    ...
    if (isMediumScreen) {
      if (balance >= 1000000) {
        return '${(balance / 1000000).toStringAsFixed(4)}M';
      } else if (balance >= 1000) {
        return '${(balance / 1000).toStringAsFixed(4)}K';
      } else if (balance >= 1) {
        return balance.toStringAsFixed(4);
