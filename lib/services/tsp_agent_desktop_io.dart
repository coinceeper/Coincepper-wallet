...
String? _resolveTspAgentBinary() {
  ...
  if (kDebugMode) {
    SecureLog.w('TspDesktop: no tsp_agent found near $exe (set TSP_AGENT_DESKTOP_BINARY)');
  }
  return null;
}
...
Future<ProcessResult?> _runTspInfoArg(String arg) async {
  ...
  try {
    ...
    SecureLog.w('TspAgentDesktop: failed to get payload key for sidecar process', error: e);
    k = null;
  }
  try {
    ...
    if (kDebugMode) {
      SecureLog.e('TspDesktop info $arg: $e');
    }
    return null;
  }
}
...
Future<String?> _getOrCreatePayloadKeyHex() async {
  ...
  String? h;
  try {
    h = await _secure.read(key: _kStorageKey);
  } catch (e) {
    SecureLog.w('TspDesktop: secure read failed', error: e);
  }
  if (h == null || h.isEmpty) {
    final b = _random32();
    ...
    try {
      ...
      SecureLog.w('TspDesktop: secure write failed', error: e);
    }
  }
  return h;
}
...
void _pipeSidecarLogs(Process proc) {
  proc.stderr.transform(const Utf8Decoder()).listen((chunk) {
    ...
    if (t.isEmpty) return;
    SecureLog.w('tsp_agent stderr: $t');
  });
}
...
Future<int> tspDesktopStart({
  ...
  try {
    ...
    SecureLog.w('TspAgentDesktop: failed to get payload key for start', error: e);
    k = null;
  }
  ...
  try {
    ...
    if (kDebugMode) {
      SecureLog.e('TspDesktop: Process.start failed: $e');
    }
    if (!kIsWeb && (Platform.isMacOS || Platform.isLinux || Platform.isWindows)) {
      SecureLog.e('TspDesktop: Process.start failed: $e');
    }
  ...
  proc.exitCode.then((code) {
    ...
    _sidecar = null;
    if (code != 0) {
      SecureLog.w('tsp_agent exited: code=$code (check config/key/ops URL)');
    }
    if (kDebugMode) {
      SecureLog.d('TspDesktop: sidecar exited with $code');
    }
  });
  if (!kIsWeb && (Platform.isMacOS || Platform.isLinux || Platform.isWindows)) {
    SecureLog.i('TspDesktop: sidecar pid=${proc.pid} (running)');
  }
  return 0;
}
...
Future<bool> tspDesktopSetDeviceKey() async {
  try {
    ...
    SecureLog.w('TspAgentDesktop: setDeviceKey failed', error: e);
    return false;
  }
}
...
Future<void> tspDesktopStop() async {
  ...
  if (s != null) {
    ...
    try {
      ...
      SecureLog.w('Error killing sidecar process', error: e);
    }
  }
}
