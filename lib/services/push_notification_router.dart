...
class PushNotificationRouter {
  ...
  static Future<void> route(RemoteMessage message) async {
    ...
    // ── Store deep-link route BEFORE showing notification ──────────────
    // This ensures that if the user taps the local notification, the
    // [NotificationHelper.onDidReceiveNotificationResponse] callback has
    // the route available immediately.
    final routePath = handleNotificationTap(message);
    NotificationHelper.storeLastNotificationRoute(routePath);
    ...
    SecureLog.d('Routing notification: type=${payload.type.value}');
  ...
  static String? handleNotificationTap(RemoteMessage message) {
    ...
    SecureLog.d('Notification tapped: type=$type');
