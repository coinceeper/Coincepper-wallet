...
class LocaleManager {
  ...
  static LocaleManager get instance => ServiceLocator.get<LocaleManager>();
  ...
  Future<void> initialize({
    ...
    SecureLog.i('LocaleManager initialized');
  }
  
  /// دریافت زبان فعلی
  Future<Locale> getCurrentLocale() async {
    try {
      final languageCode = await ServiceLocator.get<SecureStorage>().getSecureData('current_language');
      if (languageCode != null && supportedLocales.containsKey(languageCode)) {
        return supportedLocales[languageCode]!;
      }
      return defaultLocale;
    } catch (e) {
      SecureLog.e('Error getting current locale', error: e);
      return defaultLocale;
    }
  }
  ...
  Future<void> setLocale(String languageCode) async {
    try {
      if (!supportedLocales.containsKey(languageCode)) {
        SecureLog.e('Unsupported language: $languageCode');
        return;
      }
      ...
      // ذخیره زبان جدید
      await ServiceLocator.get<SecureStorage>().saveSecureData('current_language', languageCode);
      
      // فراخوانی callback
      _onLocaleChanged?.call(newLocale);
      
      SecureLog.i('Language changed to: $languageCode');
    } catch (e) {
      SecureLog.e('Error setting locale', error: e);
    }
  }
  ...
  Future<void> setLocaleFromLocale(Locale locale) async {
    try {
      ...
      if (foundLanguageCode != null) {
        ...
        SecureLog.e('Unsupported locale: $locale');
      }
    } catch (e) {
      SecureLog.e('Error setting locale from Locale object', error: e);
    }
  }
  ...
  Future<void> _loadSavedLocale() async {
    try {
      ...
      if (languageCode != null && supportedLocales.containsKey(languageCode)) {
        ...
        SecureLog.i('Loaded saved locale: $languageCode');
      }
    } catch (e) {
      SecureLog.e('Error loading saved locale', error: e);
    }
  }
  ...
  Future<void> saveLanguageSettings(Map<String, dynamic> settings) async {
    try {
      await ServiceLocator.get<SecureStorage>().saveSecureJson('language_settings', settings);
      SecureLog.i('Language settings saved');
    } catch (e) {
      SecureLog.e('Error saving language settings', error: e);
    }
  }
  ...
  Future<Map<String, dynamic>?> getLanguageSettings() async {
    try {
      return await ServiceLocator.get<SecureStorage>().getSecureJson('language_settings');
    } catch (e) {
      SecureLog.e('Error loading language settings', error: e);
      return null;
    }
  }
  ...
  Future<void> clearLanguageSettings() async {
    try {
      await ServiceLocator.get<SecureStorage>().deleteSecureData('current_language');
      await ServiceLocator.get<SecureStorage>().deleteSecureData('language_settings');
      SecureLog.i('Language settings cleared');
    } catch (e) {
      SecureLog.e('Error clearing language settings', error: e);
    }
  }
}
