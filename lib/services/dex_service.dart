...
/// سرویس DEX برای تعامل با قرارداد توکن اصلی (ERC20)
///
/// ⚠️ SECURITY: آدرس‌های قرارداد باید از طریق setContractAddresses() تنظیم شوند،
/// هرگز hardcode نکنید. در production از config on-chain یا backend دریافت می‌شود.
class DexService {
  DexService();

  DexService._();

  static DexService get instance => ServiceLocator.get<DexService>();

  final Web3Service _web3 = ServiceLocator.get<Web3Service>();

  // ═══════════════════════════════════════════════════════════════
  // آدرس‌های قرارداد — از طریق setter تنظیم می‌شوند
  // ═══════════════════════════════════════════════════════════════
  String? _dexTokenAddress;
  String? _routerAddress;
  String? _quoterAddress;

  // ABI حداقلی ERC20 برای توکن تست / محلی
  final String _dexTokenAbi = '''[
    {
  ...
  ///
  /// ⚠️ آدرس‌ها را از config backend, on-chain, یا setting دریافت کنید.
  /// هرگز hardcode نکنید.
  void setContractAddresses({
    required String dexToken,
    required String poolFactory,
    required String router,
    required String quoter,
  }) {
    _dexTokenAddress = dexToken;
    ...
    SecureLog.i('DEX contract addresses set');
  }

  // ==================== TOKEN OPERATIONS ====================

  /// دریافت موجودی توکن اصلی DEX
  Future<double> getDexTokenBalance() async {
    if (_dexTokenAddress == null) throw Exception('DEX token address not set');
    return await _web3.getTokenBalance(_dexTokenAddress!, decimals: 18);
  }
  ...
  Future<double> getUserFeeDiscount() async {
    ...
    try {
      final contract = _getDexTokenContract();
      ...
      SecureLog.e('Error getting fee discount', error: e);
      return 0.0;
    }
  }

  /// lock کردن توکن اصلی DEX
  Future<String> lockDexTokens({
    ...
    try {
      ...
      // ⚠️ Chain ID باید از Web3Service گرفته شود
      final txHash = await client.sendTransaction(
        ...
        chainId: _web3.chainId,
      );

      SecureLog.i('DEX tokens locked');
      return txHash;
    } catch (e) {
      SecureLog.e('Error locking DEX tokens', error: e);
      rethrow;
    }
  }
  ...
  Future<SwapQuote> getSwapQuote({
    ...
    try {
      ...
      SecureLog.e('Error getting swap quote', error: e);
      rethrow;
    }
  }
  ...
  Future<String> executeSwap({
    ...
    try {
      ...
      SecureLog.i('Swap executed');
      return txHash;
    } catch (e) {
      SecureLog.e('Error executing swap', error: e);
      rethrow;
    }
  }
  ...
  Future<String> addLiquidity({
    ...
    try {
      ...
      SecureLog.e('Error adding liquidity', error: e);
      rethrow;
    }
  }
  ...
  Future<String> removeLiquidity({
    ...
    try {
      ...
      SecureLog.e('Error removing liquidity', error: e);
      rethrow;
    }
  }
  ...
  DeployedContract _getDexTokenContract() {
    if (_dexTokenAddress == null) throw Exception('DEX token address not set');
    final contractAbi = ContractAbi.fromJson(_dexTokenAbi, 'ERC20');
    return DeployedContract(contractAbi, EthereumAddress.fromHex(_dexTokenAddress!));
  }
  ...
  List<DexToken> getAvailableTokens() {
    // ⚠️ آدرس توکن‌ها از config می‌آید — hardcode نکنید
    return [
      DexToken(
        address: _dexTokenAddress ?? '0x0000000000000000000000000000000000000000',
        symbol: 'CCKP',
        name: 'CoinCeeper Pool Token',
        decimals: 18,
