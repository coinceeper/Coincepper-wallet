...
class RegisterDeviceRequest {
  ...
  RegisterDeviceRequest({
    ...
    assert(anonymousId.isNotEmpty, 'DeviceID cannot be empty');
...
class PriceData {
  ...
  double? get priceAsDouble {
    try {
      ...
      if (parsed == null) {
        SecureLog.w('Failed to parse price');
      }
      return parsed;
    } catch (e) {
        SecureLog.e('Error parsing price', error: e);
      return null;
    }
  }
  ...
  double? get change24hAsDouble {
    try {
      ...
      if (parsed == null && change24h.isNotEmpty) {
        SecureLog.w('Failed to parse change24h');
      }
      return parsed;
    } catch (e) {
        SecureLog.e('Error parsing change24h', error: e);
      return null;
    }
  }
...
/// بلاکچین از API /api/blockchains
class ApiBlockchain {
  final String blockchainName;
  final String? icon;
  final String? symbol;
  ...
  factory ApiBlockchain.fromJson(Map<String, dynamic> json) {
    return ApiBlockchain(
      blockchainName: json['BlockchainName']?.toString() ?? json['name']?.toString() ?? '',
      icon: json['Icon']?.toString() ?? json['icon']?.toString(),
      symbol: json['Symbol']?.toString() ?? json['symbol']?.toString(),
      blockchainId: json['BlockchainID']?.toString() ?? json['id']?.toString(),
    );
  }
...
/// پاسخ API بلاکچین‌ها
class BlockchainsResponse {
  final List<ApiBlockchain> blockchains;
  final bool success;

  BlockchainsResponse({required this.blockchains, required this.success});

  factory BlockchainsResponse.fromJson(Map<String, dynamic> json) {
    final rawList = (json['blockchains'] as List<dynamic>? ?? []);
    final currencies = rawList
        .map((e) => ApiBlockchain.fromJson(e as Map<String, dynamic>))
        .toList();
    final success = json['success'] as bool? ?? json['status'] == 'ok';
    return BlockchainsResponse(blockchains: currencies, success: success);
  }
}
  ...
  static double? _priceFromJson(dynamic value) {
    ...
    if (value is String) {
      try {
        ...
        SecureLog.w('Could not parse price value to double');
        return null;
      }
    }
    SecureLog.w('Unexpected price value type');
    return null;
  }
}
...
/// Response from custodial getBalance API
...
/// Request model for getTransactions API
...
/// API transaction model returned from getTransactions endpoint
class ApiTransaction {
  ...
  factory ApiTransaction.fromJson(Map<String, dynamic> json) {
    return ApiTransaction(
      txHash: json['TxHash']?.toString() ?? json['txHash']?.toString(),
      from: json['From']?.toString() ?? json['from']?.toString(),
      to: json['To']?.toString() ?? json['to']?.toString(),
      amount: json['Amount']?.toString() ?? json['amount']?.toString(),
      tokenSymbol: json['TokenSymbol']?.toString() ?? json['tokenSymbol']?.toString(),
      direction: json['Direction']?.toString() ?? json['direction']?.toString(),
      status: json['Status']?.toString() ?? json['status']?.toString(),
      timestamp: (json['Timestamp'] as num?)?.toInt() ?? (json['timestamp'] as num?)?.toInt(),
      blockchain: json['Blockchain']?.toString() ?? json['blockchain']?.toString(),
      blockchainName: json['BlockchainName']?.toString() ?? json['blockchain_name']?.toString(),
      price: (json['Price'] as num?)?.toDouble() ?? (json['price'] as num?)?.toDouble(),
      temporaryId: json['TemporaryID']?.toString() ?? json['temporary_id']?.toString(),
      explorerUrl: json['ExplorerUrl']?.toString() ?? json['explorer_url']?.toString(),
      fee: (json['Fee'] as num?)?.toDouble() ?? (json['fee'] as num?)?.toDouble(),
      assetType: json['AssetType']?.toString() ?? json['asset_type']?.toString(),
      tokenContract: json['TokenContract']?.toString() ?? json['token_contract']?.toString(),
    );
  }
}

/// Response from getTransactions/history API
class GetHistoryResponse {
  final String status;
  final List<ApiTransaction> transactions;

  GetHistoryResponse({required this.status, required this.transactions});

  factory GetHistoryResponse.fromJson(Map<String, dynamic> json) {
    return GetHistoryResponse(
      status: json['status'] as String? ?? json['Status'] as String? ?? 'error',
