...
class _PanelScreenState extends State<PanelScreen>
  ...
  static const double _kBottomMenuClearance = 82;
  ...
  Widget _buildBody(ClientPanelProvider provider, Color primary) {
    ...
    if (!provider.authenticated && provider.needsWebPin) {
...
class _InviteCodeGateState extends State<_InviteCodeGate> {
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.lock_outline_rounded, size: 64, color: primary),
  ...
  Future<void> _submit() async {
    ...
    final addr =
        context.read<ClientPanelProvider>().boundPanelAddress ?? '';
    final ok = await context
        .read<ClientPanelProvider>()
        .submitInviteCode(code, addr);
    if (!mounted) return;
    setState(() => _loading = false);
    if (!ok) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('panel.invite_parse_empty'.tr())),
      );
    }
  }
}
                ...
                child: ElevatedButton(
                  onPressed: _pinBusy
                      ...
                      : () async {
                          ...
                          await context
                              .read<ClientPanelProvider>()
                              .submitWebPin(_pinCtrl.text, null);
