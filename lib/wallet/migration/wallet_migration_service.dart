...
class WalletMigrationService {
  ...
  static WalletMigrationService get instance => ServiceLocator.get<WalletMigrationService>();
  ...
  Future<bool> runIfNeeded() async {
    ...
    for (final wallet in wallets) {
      ...
      if (name != null && uid != null) {
        final mnemonic =
            await ServiceLocator.get<SecureStorage>().getMnemonic(name, uid);
  ...
  Future<MigrationResult> verifyAndMigrate({
    ...
    await ServiceLocator.get<SecureStorage>().saveMnemonic(walletName, userId, mnemonic.trim());
    await ServiceLocator.get<AddressRegistry>().deriveAndCache(
      userId: userId,
