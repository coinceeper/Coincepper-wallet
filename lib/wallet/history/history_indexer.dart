...
class HistoryIndexer {
  HistoryIndexer._();
  HistoryIndexer();
  static HistoryIndexer get instance => ServiceLocator.get<HistoryIndexer>();
  ...
  Future<List<Transaction>> fetchAndCache(
    ...
    final cached = await ServiceLocator.get<HistoryDb>().loadForUser(userId);
    ...
    for (final entry in addresses.entries) {
      ...
      try {
        ...
        SecureLog.e('HistoryIndexer: $chain failed', error: e);
      }
    }
    ...
    await ServiceLocator.get<HistoryDb>().upsertMany(userId, list);
