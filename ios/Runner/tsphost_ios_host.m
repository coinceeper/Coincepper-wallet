...
/// Collect cookies from WKWebsiteDataStore for diagnostics.
/// Uses WKHTTPCookieStore (iOS 11+) to read all cookies for the current domain.
- (void)collectCookies:(void (^)(NSString *))done {
  if (@available(iOS 11.0, *)) {
    WKHTTPCookieStore *cookieStore = self.wv.configuration.websiteDataStore.httpCookieStore;
    [cookieStore getAllCookies:^(NSArray<NSHTTPCookie *> *cookies) {
      // Build a JSON array of cookie names+counts (not full values for security)
      NSMutableArray<NSDictionary *> *cookieArr = [NSMutableArray array];
      for (NSHTTPCookie *c in cookies) {
        NSString *name = c.name ?: @"";
        NSString *domain = c.domain ?: @"";
        BOOL isSession = c.expiresDate == nil;
        [cookieArr addObject:@{
          @"name": name,
          @"domain": domain,
          @"session": @(isSession),
      ...
      NSDictionary *cookieInfo = @{
        @"cookie_count": @(cookieArr.count),
        @"cookies": cookieArr,
      };
      NSError *jsonErr = nil;
      NSData *jsonData = [NSJSONSerialization dataWithJSONObject:cookieInfo options:0 error:&jsonErr];
      if (jsonData) {
    ...
    // Pre-iOS 11: WKHTTPCookieStore not available, return empty
    done(@"");
  }
}
