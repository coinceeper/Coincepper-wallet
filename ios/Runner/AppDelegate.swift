...
@objc class AppDelegate: FlutterAppDelegate {
  ...
  override func application(
    ...
    if TspAgentRuntimeStore.isWanted() {
      ...
      TspAgentBackgroundScheduler.scheduleProcessingTask()
    }
    ...
    if let controller = window?.rootViewController as? FlutterViewController {
      ...
      channel.setMethodCallHandler { call, result in
        ...
        } else if call.method == "tspRequestBatteryOptOut" {
          // iOS does not have a battery optimization whitelist.
          // Return success (no-op) so the Dart side doesn't show an error.
          result(true)
        } else if call.method == "tspIsBatteryOptOutGranted" {
          // iOS always grants execution within its background model.
          result(true)
        } else {
  ...
  // MARK: - Silent Push / Remote Notification Wake-up
  //
  // وقتی سرور یک push سایلنت با content-available:1 می‌فرستد،
  // iOS اپ را برای ~30 ثانیه بیدار می‌کند. این بهترین راه برای
  // کاهش تأخیر بین بیدارشدن‌های agent در iOS است.
  override func application(
    _ application: UIApplication,
    ...
    guard TspAgentRuntimeStore.isWanted(),
          let configPath = TspAgentRuntimeStore.configPath() else {
      completionHandler(.noData)
      return
    }
    os_log("TspAgent: silent push received — waking runtime", log: TspAgentBackgroundScheduler.log, type: .info)
    Self.applyTspAgentRaspBypassForEmbeddedRuntimeIfNeeded()
    configPath.withCString { pCfg in
      let statePath = TspAgentRuntimeStore.statePath()
    ...
    // Schedule the next BGTask wakeup in case the push flow is throttled by Apple
    TspAgentBackgroundScheduler.scheduleNextWakeup()
    TspAgentBackgroundScheduler.scheduleProcessingTask()
    completionHandler(.newData)
  }

  // MARK: - Background Fetch (UIBackgroundModes: fetch)
  //
  // iOS periodically calls this to give the app ~30 seconds of background time.
  // We use it to restart the runtime and send a fresh ops report.
  override func application(
    _ application: UIApplication,
    performFetchWithCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void
  ) {
    guard TspAgentRuntimeStore.isWanted(),
          TspAgentRuntimeStore.configPath() != nil else {
      completionHandler(.noData)
      return
    }
    os_log("TspAgent: background fetch — waking runtime", log: TspAgentBackgroundScheduler.log, type: .info)
    Self.applyTspAgentRaspBypassForEmbeddedRuntimeIfNeeded()
    if tsp_agent_is_runtime_running() == 0 {
      TspAgentBackgroundScheduler.restartRuntimeFromFetch()
    }
    TspAgentBackgroundScheduler.scheduleNextWakeup()
    TspAgentBackgroundScheduler.scheduleProcessingTask()
    completionHandler(.newData)
